...
\begin{center}
\includegraphics[width=4.2cm]{figs/logos-cover.jpg}
\end{center}

\begin{center}
\begin{minipage}{0.86\textwidth}
\footnotesize
\textbf{Note on authorship.} \emph{This summary} was drafted in 2026 by Claude
(Anthropic), working from the dissertation's original \LaTeX{} sources, the
published source code, and the bibliographic record, and was directed, corrected
and approved by the author. Its claims are deliberately tied to things a reader
can check independently --- measurements from the evaluation chapter,
identifiers from the sources, dates from the bibliography --- so that the
drafting need not be taken on trust. The companion errata carries the same note.

\emph{The dissertation itself} is not AI-assisted work. It was submitted in
2005, close to two decades before such tools existed, and is entirely the
author's own.
\end{minipage}
\end{center}
...
The dissertation it summarises is 586 pages long and written in German. For most
of the people who might now have a reason to know what is in it --- colleagues in
industry, researchers working on description logics, graph databases, query
engines or reasoning systems --- that makes it effectively unreadable. The work
is open access and carries a DOI, and is still almost impossible to assess.

This document is therefore a \textbf{record}: what the work set out to do, what
was actually built, what was measured, and what it achieved. It is intended to
give a reader enough context to judge the work and the background it reflects,
without reading the German and without taking anything on trust.

That last point governs how it is written. Claims are tied to things a reader can
check --- measurements from the evaluation chapter with their figures, class and
method names from the published sources, dates from the bibliography. Where a
technique disappointed, the measured factor is given. Where something was
designed but not implemented, that is said. The companion errata lists defects
found on re-reading, including one that turned out to be a defect in the
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figs/dlmaps.png}
\caption{The DLMAPS map viewer on the DISK --- the digital city map of
Hamburg-Öjendorf. Map objects are instances of spatial datatypes in a spatial
index and simultaneously ABox individuals, so one query can combine thematic
conditions (\emph{Wohnen}, \emph{Gew\"asser}, \emph{Friedhof}) with qualitative
spatial ones. The thematic object-key catalogue is at left, the selected
object's segments at right.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.62\textwidth]{figs/owltree.png}
\caption{The \textsc{OWL Tree}, a browser front end to the Semantic Web
retrieval service, built by \textbf{Matthias Flege, Thomas Hirsch, Torben Rebhan
and Kamil Sokolski} in a project seminar at TUHH, supervised by the author.
Tree-structured conjunctive queries are assembled from concepts and roles ---
here \texttt{FullProfessor}, \texttt{teacherOf}, \texttt{GraduateCourse} over the
LUBM vocabulary --- submitted to a RacerPro server as nRQL, and the result tuples
presented for inspection. The team also extended the LUBM generator so that
individuals were \emph{real} web resources rather than synthetic URIRefs ---
TUHH staff homepages for \texttt{employee}, course pages for \texttt{course},
STS publication PDFs for \texttt{publication} --- making every answer a
clickable link to the thing it denoted.}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.74\textwidth]{figs/rcc-browser.png}
\caption{The substrate browser on an RCC substrate --- here an RBox ABox of 8
nodes and 64 edges, reported consistent. Every edge carries the qualitative
spatial relation holding between two regions (\textsc{pp}, \textsc{ppi},
\textsc{eq} and their inverses); there are no coordinates anywhere, which is
what makes the reasoning qualitative. The lower pane shows the internals of a
node label --- the expanded and unexpanded concept sets, the \textsc{ands},
\textsc{ors}, \textsc{somes}, \textsc{alls} and number restrictions --- i.e.\ the
tableau state of that node, inspectable while the prover runs.}
\end{figure}
...
\paragraph{Two formal semantics, deliberately.}
The query language is then given a \emph{second}, independent formal semantics,
and the pair is the point.

The first is \textbf{logic-based}: the extension of a query is defined through
entailment, by translating bodies into first-order formulae via $\Phi$ and asking
what the substrate entails. It says \emph{what the correct answer is}, and says
it in the same currency as the substrate's own semantics and the description
logic underneath. A logic-programming reading is given alongside it.

The second is \textbf{algebraic}. The extension of a body is constructed
directly by set operations over the extensions of its sub-bodies: $\wedge$,
$\vee$ and $\setminus$ become $\cap$, $\cup$ and $\setminus$ on sets of $n$-ary
tuples. Because those operations need uniform arity, two further operations are
defined --- \emph{projection} to reduce dimensionality and its inverse,
\emph{cylindrification}, to raise it --- together with diagonals and the
$\mathbf{1}$-vector. That is the vocabulary of cylindric algebra, and the
justification offered is the known equivalence of non-recursive negation-free
Datalog with relational algebra.

The division of labour is what matters. The logical semantics is the
\emph{specification}: it fixes what an answer means, and it is what lets query
subsumption and satisfiability be posed as inference problems at all. The
algebraic semantics is \emph{constructive} --- the thesis notes it is
correspondingly implementation-near --- and it is essentially the blueprint the
query engine of Chapter 5 follows, which is why that engine's optimiser reasons
about generators, testers and cross products rather than about entailment.

The two are presented as alternative accounts of the same language rather than
proved equivalent; the argument for their agreement rests on the Datalog and
relational-algebra correspondence. Giving both is unusual, and it is the reason
the language can be specified logically and evaluated like a database query
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figs/dls.png}
\caption{The $DL$ axis of the MiDeLoRa space as an inheritance lattice. Feature
mixins on the left (\texttt{DL-WITH-TRANSITIVE-ROLES},
\texttt{DL-WITH-INVERSE-ROLES}, \texttt{DL-WITH-BLOCKING}, \ldots) compose by
multiple inheritance into the named logics on the right ---
$\mathcal{ALCH}$, $\mathcal{ALCHN}$, $\mathcal{ALCHIF}_{R^+}$, and the spatial
$\mathcal{ALCI}_{RCC}$. A prover is defined at whichever node is appropriate and
inherits its rules along the lattice.}
\end{figure}
...
There is a second speed to this. For \emph{query atoms} the method is resolved at
runtime, because the substrate a query touches is not known until it runs. For
\emph{tableau rules} the same lattice is resolved once, at composition time: a
rule is conceptually a multimethod on \emph{(language class, ABox class)}, but
what the dispatch returns is a \emph{code template}, and the templates are
composed into a prover before it is ever run. Selection by dispatch, execution by
compiled code --- the dispatch cost is paid once per prover rather than once per
rule application.
...
\subsection{Operational design: lifecycle, concurrency, incrementality}

The requirements the thesis sets for the engine are operational rather than
semantic, and the list reads more like a database query manager than a research
prototype: incremental answering; \textbf{multi-user operation with queries
processed concurrently}; \textbf{lifecycle management}, with queries as objects
reachable and manipulable through an API; and resource-configurable answering ---
proactive or on-demand tuple computation, incomplete modes, and phased
computation where cheap tuples are found first and expensive ones only on demand.

Three mechanisms deliver this.

\textbf{Continuation-passing style.} Processing functions are written so that
each atom's function calls a \emph{continuation} to extend the current binding.
That is what makes results streamable, suspendable and abortable partway
through --- and it is precisely what the code-template compiler optimises away,
nesting the continuations into a single function at the query root so the
\texttt{funcall} chain disappears at answering time.

\textbf{A process pool.} The sources carry a dedicated process pool, and the
control surface is explicit: \texttt{prepare-to-run-query},
\texttt{query-running-p}, \texttt{abort-query}, \texttt{abort-process},
\texttt{wait-for-request-or-abort} --- with the same set for rules. Queries run
in the background without blocking the caller, and can be cancelled while
running.

\textbf{Queries as first-class objects.} A query is not a string submitted and
forgotten. It is an object with a state, a lifecycle and an API: prepare it, start
it, ask it for the next tuple, inspect what it is doing, abort it, or come back
to it later. Incremental retrieval is a separate module in the sources
(\texttt{get-next-tuple}).

\begin{note}
\textbf{Worth noticing in a 2005 reasoner.} Incremental and abortable evaluation,
concurrent execution of independent queries, non-blocking background processing,
and a managed query lifecycle are not what description logic systems of the
period typically offered --- reasoners were usually synchronous, single-query,
run-to-completion. Combining them with a reasoner-backed query semantics is the
part that was unusual, and it is what made the two-phase and incomplete modes
operationally meaningful: a mode that trades completeness for resources is only
useful if the caller can also stop, resume, and ask for more.
\end{note}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{figs/inspector.png}
\caption{The DLMAPS \emph{Query Inspector}, answering \emph{\enquote{show me all
residential areas adjacent to a park}} --- in DISKQL,
\texttt{(AND (?*X WOHNGEBIET) (?*Y PARK) (?*X ?*Y ADJACENT))}. Each answer tuple
is drawn as its own map crop with the bindings of \texttt{?*X} and \texttt{?*Y}
highlighted, so a result set is inspected visually rather than read as a table.
Note the \emph{Abort Search} button and the page counter: results arrive
incrementally and the search can be stopped while running, which is the
operational design of the engine made visible.}
\end{figure}
...
and the sources refine that usefully. The fine-grained causal machinery is
unused: \texttt{precondition-for-actions} and its inverse are declared on
\texttt{abox-item} but never written or read, and the two-phase marking
algorithm has no implementation.

The \emph{principle}, however, ships --- by another route. Under
\textbf{retention of deterministic assertions}, MiDeLoRa deliberately does not
roll back to the start of a tableau expansion after a satisfiability test.
Instead it removes only the assertions $A$ with
$\mathit{choice\_points}(A) \neq \emptyset$ --- those that depend on a choice ---
and keeps the deterministic remainder, which is logically implied and so remains
valid. The next satisfiability test then skips re-expanding what survived.
Chapter 7 measures the effect: the average ABox unsatisfiability test falls from
\textbf{12\,s to about 3.7\,s}.

So dependency-directed retraction is real here, at the granularity of tableau
expansions and keyed on choice-point sets. What stayed a sketch is the per-clash
variant driven by the causal chain.

The chapter also asks the right sceptical question of itself: whether thoroughgoing OOP will
...
Implementation covers set representation, the command objects and history,
provers and rule application chains, and the representation of models for the
model merging test.

\subsection{The rule definition language}

The construction kit is driven by a \textbf{domain-specific language} for writing
tableau rules --- a Common Lisp macro language implementing a \emph{five-port
model}. It does not remove the need to program rules; it supplies the
abstractions that make rules composable.

A rule is a point on the $DL \times K$ plane of the MiDeLoRa space, written as
what is conceptually a CLOS multimethod on a language class and an ABox class:

\begin{center}
\texttt{(defrule or-expansion (dl abox) …)}
\end{center}

Rules therefore \emph{inherit along both axes} by ordinary CLOS semantics --- a
rule for a more expressive logic inherits the implementation from a less
expressive one unless it overrides it --- and where the space's structure does not
give the right answer, a rule can request another's implementation outright with
\texttt{:inherit-from}. The two outcomes of a rule are marked in its body by
\texttt{+insert-positive-code+} and \texttt{+insert-negative-code+}; the re-entry
port is implicit in the return of control, and the backtracking port is
\texttt{return-from prover}, which passes branch-point identifiers up to the
parent node in the search tree.

Provers are then assembled from those chains by a second macro,
\texttt{define-prover}, whose argument list \emph{is} a coordinate in the
MiDeLoRa space $T \times DL \times K$, and whose body names the three operational
phases. An $\mathcal{ALCH}$ ABox satisfiability prover reads in full:
...
\noindent
That is the whole prover. The tableau strategy is visible as structure rather
than buried in control flow: \texttt{perform} fires a rule and dispatches on its
ports, so \texttt{:positive} and \texttt{:negative} are literally the exits of the
rule boxes in the chain diagram opposite. \texttt{restart-main} creates a fresh incarnation after a
nondeterministic rule fired --- the stack discipline the architecture requires ---
and a phase left undefined simply inherits by CLOS semantics, or can request
another prover's implementation with \texttt{:inherit-from}.

\begin{sidewaysfigure}
\centering
\includegraphics[width=0.96\textheight]{figs/prover-chain.png}
\caption{A complete prover as a wired rule application chain --- the main phase
of an $\mathcal{ALC}$ ABox satisfiability prover. Each rule is a box with the
five ports: two entries (\emph{main}, open diamond at left, and \emph{re-enter},
open diamond at the bottom) and three exits (\emph{positive} and \emph{negative},
filled diamonds at right, and \emph{backtrack}, filled diamond at the bottom).
The strategy is the wiring: \textsc{deterministic-expansion} runs first, its
\textsc{neg} exit passes to \textsc{or-expansion}, whose \textsc{neg} exit passes
to \textsc{some-expansion}, whose \textsc{neg} exit is \emph{success} --- nothing
left to expand. Each \textsc{pos} exit loops back to the top, creating the fresh
incarnation a nondeterministic rule needs, and the backtrack exits run to
\emph{fail}. This is the same prover as the \texttt{define-prover} listing
above, drawn.}
\end{sidewaysfigure}

The performance decision is the interesting one. \texttt{defrule} does not define
the rule directly --- it defines a method returning a \textbf{code template}, and
the rule application chain selects templates by $DL \times K$ at
\emph{composition time}. Late binding chooses the implementation once, when the
prover is assembled, instead of on every rule application. Because rules are
templates rather than functions, the compiler is then free either to emit a call
to the next rule or to \emph{inline its code} --- \enquote{code unfolding} ---
though only when the next rule is deterministic, since a nondeterministic rule
needs a fresh incarnation on the search stack to preserve its context.

\begin{note}
\textbf{One measurement that is missing.} Whether compiling assembled prover
definitions, rather than leaving them to be evaluated, actually pays is a natural
question for this design, and the thesis does not answer it --- no such
comparison is reported. The nearest published figure is from the query side,
where the same code-template technique was measured at $1.568\times$ and judged
\enquote{not as effective as hoped}. Whether provers behave the same way is
open; the rule-application path is much hotter than the query-translation path,
so the result need not transfer.
\end{note}
...
\subsection{The dated record}

Questions of precedence are best settled by dates, so here is the publication
record underlying the thesis, taken from its own bibliography.

\begin{center}
\small
\begin{tabular}{@{}l p{6.1cm} p{4.3cm}@{}}
\toprule
\textbf{Year} & \textbf{Work} & \textbf{What it establishes} \\
\midrule
2000 & $\mathcal{ALCRA}$ --- $\mathcal{ALC}$ with Role Axioms; Decidable and Undecidable Extensions of $\mathcal{ALC}$ & the spatial DL line opens \\[0.6ex]
2000--01 & Obstacles on the Way to Spatial Reasoning with Description Logics & undecidability results \\[0.6ex]
2002 & On Spatial Reasoning with Description Logics & position paper \\[0.6ex]
2003 & Qualitative Spatial Reasoning with the $\mathcal{ALCI}_{RCC}$-family & the decidable fragments \\[0.6ex]
2003 & Some Practical Issues in Building a Hybrid Deductive Geographic Information System & guiding domain 1 \\[0.6ex]
\textbf{2004} & \textbf{Querying the Semantic Web with Racer + nRQL} & \textbf{nRQL in public} \\[0.6ex]
2004 & Extended Query Facilities for Racer & the query facilities \\[0.6ex]
\textbf{2005} & \textbf{A High Performance Semantic Web Query Answering Engine} & \textbf{the engine and its optimiser} \\[0.6ex]
2006 & On the Scalability of Description Logic Instance Retrieval & the retrieval optimisations \\[0.6ex]
2006 & A Flexible DL-based Architecture for Deductive Information Systems & the framework \\[0.6ex]
2007 & Design Principles for User Friendly, Interactive DL interfaces (OWLED) & the GUI work \\[0.6ex]
2009 & Flexible Software Architectures for Ontology-Based Information Systems, JAL 7(1), 75--99 & the overview; accepted 2007 \\
\bottomrule
\end{tabular}
\end{center}

Set against that, the dissertation was \textbf{submitted in 2005};
\textbf{SPARQL 1.0} became a W3C Recommendation in \textbf{January 2008}; and
\textbf{SPARQL 1.1}, with subqueries, \texttt{NOT EXISTS}, \texttt{MINUS} and the
\textbf{entailment regimes} specification, in \textbf{March 2013}.

\begin{note}
\textbf{Read this record honestly, in both directions.} The dates are what they
are: nRQL was described in print in 2004 and its optimising engine in 2005, and
the features SPARQL reached in 2013 were in it then. That is chronological
precedence and it is checkable.

It is also worth saying that most of these are \emph{workshop} papers --- the
Description Logic workshop, KRDB, OWLED --- with one journal article at the end.
Workshop proceedings are cited less, indexed less, and read less than journal or
conference publications, which is one concrete reason this work is less visible
than its dates would suggest. It is the sort of thing worth knowing about one's
own record rather than being surprised by.

And precedence is not influence. Similar ideas arrived later in other systems;
whether they arrived independently, by way of this work, or through the shared
background all of it drew on, the dates alone cannot say.
\end{note}
...
2008}; the three-year gap was examination time, not work. (Logos Verlag
catalogues the book edition as 2009, so both years appear in the wild --- the
dissertation is 2008, the book 2009.)
...
\section{Did it deliver on its title?}

The thesis set itself a programme in its own title: \emph{flexible and
configurable software architectures for data-intensive ontology-based information
systems}. Taking the terms one at a time, and reading them as an engineer would
rather than as a marketing department might:

\textbf{Flexible --- yes, and demonstrably.} This is the best-supported claim in
the work, because it was made falsifiable. The test was whether one framework
could serve two domains chosen to pull in opposite directions: LUBM, which is
large, shallow and conjunctive, and the DISK, which is small, deep and spatially
hybrid. It served both. One data model was instantiated as description logic
ABoxes, RDF graphs, RCC networks, data substrates and geometric substrates; four
query languages were built on one framework; provers were assembled along two
language strands plus task-specific ones. \enquote{Regions rather than points}
turned out to be an operational statement about a dispatch lattice, not a slogan.

\textbf{Configurable --- yes.} Configurability is shown the only way it can be,
by exhibiting configurations that work: the rule definition language for
building provers, the substrate and atom class lattice, and nRQL's four modes
plus its two-phase and incremental operation, which make the resource/completeness
trade-off a runtime choice rather than a design-time one.

\textbf{Software architectures --- yes.} Three frameworks that share one data
model, an explicit comparison of layered, semi-integrated and integrated designs,
and a reasoned commitment to the integrated one on cohesion grounds.

\textbf{Data-intensive --- partly, and the thesis says so.} This is the weakest
term and the one where the honest answer is qualified. LUBM at ten universities on
2006 hardware with apparently linear scaling is a real result. But MiDeLoRa in
complete mode managed one university --- 17,174 individuals --- with a 350.8\,s
initial satisfiability test, the DISK needed the smaller maps, and the thesis
describes its own successes as \enquote{fragile}, resting on incomplete methods
that settle 99.98\,\% of instance tests by luck as much as design. Its own verdict
is that the satisfiability test needs one to two orders of magnitude of speedup
before the results are robust. \emph{Data-intensive} is achieved in the sense that
scalability was addressed, measured, and shown to be non-exponential --- not in
the sense that these systems would stand next to a relational database.

That distinction deserves precision rather than modesty. DLMAPS and MiDeLoRa were
research prototypes and are described as such. nRQL was not: it shipped inside a
commercial reasoner, was used in production, and was the first Semantic Web
repository to answer the LUBM queries completely. One component crossed into real
use; the others demonstrated that the framework could produce them.

\textbf{Overall.} Three of the four terms are delivered convincingly and the
fourth partially and honestly. A fair summary is that the thesis achieved the
architectural programme it set itself and fell short of the performance
programme --- which is the ordinary outcome when a research prototype is measured
against systems with orders of magnitude more engineering behind them, and is
stated plainly in the evaluation rather than glossed.

The work's most durable contribution is arguably not announced in the title at
all. The reasoner construction kit of Chapter 6 --- provers as points in a space,
the tableau as a first-class inspectable object, the proof process itself
represented as data --- is the part that had no real counterpart then and still
has few now.
