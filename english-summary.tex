...
\newenvironment{note}
  {\begin{trivlist}\item[]\leftskip=1.5em \rightskip=1.5em \small}
  {\end{trivlist}}
...
query engine and the reasoner inseparable.

\textbf{Negation as failure} is then added as NAF atoms with their own semantics,
distinguished carefully from the classical negation available inside concepts.
Complex queries follow: abstract syntax, the functions $\mathit{objects}(k)$ and
$\mathit{vars}(k)$, the extension of $\Phi$ to query bodies, the semantics of a
query, and a $\models_{NAF}$ relation.
...
\item \textbf{True dependency-directed backtracking (DDB).} Contemporary
  ...
  \texttt{precondition-for-actions}, with an inverse
  \texttt{postcondition-of-actions} for navigation in both directions. On a
...
\item \textbf{shacl-nrql-comparison} --- a retrospective analysis of nRQL against
