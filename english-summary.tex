...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.82\textwidth]{figs/five-port.png}
\caption{The five-port model. A rule has two entries --- \textsc{main entry} and
\textsc{re-enter entry} --- and three exits: \textsc{positive} (the rule applied),
\textsc{negative} (it did not), and \textsc{backtrack}. A rule application chain
is built by wiring one rule's exits to the next rule's entries, which is what
makes a search strategy something you compose rather than something you code.}
\end{figure}
