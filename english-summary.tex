...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.86\textwidth]{figs/rcc-browser.png}
\caption{An RCC substrate rendered by the substrate browser. Nodes carry region
names; each edge is labelled with the qualitative spatial relation that holds
between the two regions --- \textsc{tpp} (tangential proper part),
\textsc{ntpp} (non-tangential proper part), \textsc{po} (partial overlap),
\textsc{eq}. This is the extensional layer a spatial-thematic query is evaluated
against: no coordinates, only the relations.}
\end{figure}
